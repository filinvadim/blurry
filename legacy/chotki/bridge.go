...
// Package chotki_legacy snapshots the Blurry/Chotki compatibility layer
// that lived in the live tree until Blurry was refactored into a pure
// libp2p+CRDT+badger key/value store.
//
// Nothing here is built by default — pass `-tags chotki_legacy` to opt
// in. The code is kept as-is for archival reference; it depends on
// Blurry internals (Store, ID, FieldSpec, …) that are also parked under
// this package, so it still compiles standalone with the tag.
